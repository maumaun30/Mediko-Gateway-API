...
if (TESTING_MODE) {
  console.warn("⚠️  TESTING_MODE is ON — rate limit, time trap, and cooldown are disabled.");
}
...
app.use((req, _res, next) => {
  log("info", "request_received", { method: req.method, path: req.path, ip: req.ip });
  next();
});
...
db.getConnection((err, conn) => {
  if (err) {
    ...
    conn.release();
    ensureContactTable();
    ensureReturnsTable(); // ensureStatusColumn is called inside after table is ready
  }
});
  ...
  db.query(
    ...
    (err, rows) => {
      ...
      db.query(`ALTER TABLE ${table} ADD COLUMN ${name} ${definition}`, (alterErr) => {
        if (alterErr) log("error", "migration_failed", { table, column: name, message: alterErr.message });
        else log("info", "migration_applied", { table, column: name });
      });
    },
  );
}
...
// Runs after ensureReturnsTable succeeds to avoid race conditions
function ensureStatusColumn() {
  ...
  ensureColumn("discount_id", "VARCHAR(128) NULL");        // GraphQL GID for the discount node
  ensureColumn("discount_code_id", "VARCHAR(128) NULL");   // GraphQL GID for the redeem code
  ...
  db.query(
    ...
    (err) => {
      if (err) log("error", "migration_returns_table_failed", { message: err.message });
        ...
        ensureStatusColumn(); // safe to run now — return_requests table exists
      }
    },
  );
}
...
transporter.verify((err) => {
  if (err) log("error", "mailer_verify_failed", { message: err.message });
  else log("info", "mailer_ready");
});
...
// REST helper — still used for customer lookup/create
async function shopifyFetch(method, pathSuffix, body) {
  ...
  if (!res.ok) {
    const msg = (json && (json.errors || json.error)) || `Shopify ${res.status}`;
...
// GraphQL helper — used for discount creation/deletion/usage
async function shopifyGraphQL(query, variables = {}) {
  ...
  // Surface userErrors from mutation responses
...
async function findOrCreateShopifyCustomer(applicant) {
  ...
  const search = await shopifyFetch("GET", `/customers/search.json?query=${emailQuery}`);
...
async function createDiscountForApplicant(applicant) {
  ...
  const variables = {
    basicCodeDiscount: {
      ...
      customerGets: {
        value: {
          percentage: DISCOUNT_PERCENTAGE / 100, // e.g. 0.20 for 20%
        },
        items: {
          all: true,
        },
  ...
  log("info", "shopify_discount_created", {
    appId: applicant.id,
    discountId: node.id,
    codeId: codeNode.id,
    code: codeNode.code,
    combinesWith: node.codeDiscount.combinesWith,
    usageLimit: node.codeDiscount.usageLimit,
    appliesOncePerCustomer: node.codeDiscount.appliesOncePerCustomer,
  });

  return {
    code: codeNode.code,
    discountId: node.id,           // gid://shopify/DiscountCodeNode/...
    discountCodeId: codeNode.id,   // gid://shopify/DiscountRedeemCode/...
  };
}
...
async function deleteDiscount(discountId) {
  ...
  `;
  await shopifyGraphQL(mutation, { id: discountId });
  log("info", "shopify_discount_deleted", { discountId });
}

// ── Email helpers ─────────────────────────────────────────────────────────────
async function sendApplicationEmails(application, reqFiles, metadataStr) {
  const { full_name, email_address, id_number, birthday, contact_number, insertId } = application;
  ...
  const attachments = reqFiles ? reqFiles.map((f) => ({ filename: f.originalname, path: f.path })) : [];
  ...
  const applicantMail = {
    ...
    html: `
      <div style="font-family: sans-serif; line-height: 1.6; color: #333;">
        ...
        ID Number: ${escapeHtml(id_number)}<br>
  ...
  const adminMail = {
    ...
    cc: process.env.ADMIN_CC?.trim() ? process.env.ADMIN_CC.split(",") : [],
    ...
    html: `
      ...
      <table border="1" style="border-collapse: collapse; width: 100%; max-width: 600px; font-family: sans-serif;">
        ...
        <tr><td style="padding: 8px;"><strong>Birthday</strong></td><td style="padding: 8px;">${escapeHtml(birthday)}</td></tr>
        ...
        <tr><td style="padding: 8px;"><strong>ID Number</strong></td><td style="padding: 8px;">${escapeHtml(id_number)}</td></tr>
      </table>
      ${metaTable(meta)}
    `,
  };
  ...
  try {
    await Promise.all([transporter.sendMail(applicantMail), transporter.sendMail(adminMail)]);
    log("info", "emails_sent", { appId: insertId });
  } catch (err) {
    log("error", "email_send_failed", { appId: insertId, message: err.message });
  }
}
...
async function sendApprovalEmail(applicant, code) {
    ...
    html: `
      <div style="font-family: sans-serif; line-height: 1.6; color: #333;">
        ...
        <div style="background-color: #f0fdf4; border: 1px solid #16a34a; padding: 20px; border-radius: 8px; margin: 20px 0; text-align: center;">
          ...
          <p style="margin: 0; font-size: 28px; font-weight: bold; color: #15803d; letter-spacing: 2px;">${escapeHtml(code)}</p>
          <p style="margin: 12px 0 0 0; font-size: 13px; color: #555;">${DISCOUNT_PERCENTAGE}% off — <strong>one-time use only</strong>.</p>
        </div>
        ...
        <ol>
          <li>Add all your items to the cart on <a href="https://mediko.ph">mediko.ph</a>.</li>
          <li>At checkout, enter the email address you used to apply: <strong>${escapeHtml(applicant.email_address)}</strong>.</li>
          <li>Paste the code <strong>${escapeHtml(code)}</strong> in the discount field and apply.</li>
          <li>Complete your purchase — the code will be consumed after your order is placed.</li>
        </ol>

        <p style="font-size: 13px; color: #777;">Keep this email for your records. If you experience any issues, please contact our support team before attempting to use the code.</p>
        <p>Stay healthy!<br><strong>Mediko.ph Team</strong></p>
      </div>
    `,
  };
  ...
  try {
    ...
    log("error", "approval_email_failed", { appId: applicant.id, message: err.message });
  }
}
...
async function sendRejectionEmail(applicant, reason) {
  ...
  try {
    ...
    log("error", "rejection_email_failed", { appId: applicant.id, message: err.message });
  }
}
...
async function sendReturnEmails(returnReq, files, metadataStr) {
  const { id, full_name, email_address, contact_number, order_number, items_to_return, reason } = returnReq;
  let meta = {};
  try {
    meta = typeof metadataStr === "string" ? JSON.parse(metadataStr) : metadataStr || {};
  } catch { meta = {}; }

  const attachments = files ? files.map((f) => ({ filename: f.originalname, path: f.path })) : [];
...
app.post("/api/apply", submitLimiter, upload.array("id_photos", 2), (req, res) => {
  ...
  const runCooldown = (cb) =>
    TESTING_MODE ? cb(null, []) : db.query(cooldownSql, [email_address, id_number], cb);
  ...
  runCooldown((err, rows) => {
    ...
    if (rows.length > 0) {
      ...
      return res.status(200).json({ success: true, message: "We already received this application. No need to submit again!" });
    }

    const photoPaths = req.files ? req.files.map((f) => f.path).join(",") : "";

    db.query(
      `INSERT INTO applications (full_name, birthday, contact_number, email_address, id_number, id_photo_path, metadata)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [full_name, birthday, contact_number, email_address, id_number, photoPaths, metadata],
      (err, result) => {
        if (err) {
        ...
        log("info", "application_saved", { appId: result.insertId, email: email_address, ip: req.ip, photoCount: req.files.length });
        ...
        sendApplicationEmails(
          { full_name, email_address, id_number, birthday, contact_number, insertId: result.insertId },
          req.files,
          metadata,
        );
      },
  ...
  try {
    if (!TESTING_MODE) {
      const dup = await queryAsync(
        `SELECT id FROM contact_submissions WHERE email_address = ?
         AND created_at > DATE_SUB(NOW(), INTERVAL 2 MINUTE) LIMIT 1`,
        [email_address],
      );
      if (dup.length) {
        log("info", "contact_duplicate_ignored", { email: email_address });
        return res.status(200).json({ success: true, message: "We already received your message. We'll be in touch soon!" });
      }
    }

    const result = await queryAsync(
      `INSERT INTO contact_submissions (full_name, email_address, contact_number, message, metadata)
       VALUES (?, ?, ?, ?, ?)`,
      [full_name, email_address, contact_number, message, metadata],
    );

    log("info", "contact_saved", { id: result.insertId, email: email_address, ip: req.ip });

    res.json({ success: true, message: "Message sent! Please check your email for confirmation.", id: result.insertId });

    sendContactEmails({ id: result.insertId, full_name, email_address, contact_number, message }, metadata);
  } catch (err) {
    log("error", "contact_insert_failed", { message: err.message, email: email_address });
    res.status(500).json({ error: "Failed to save message." });
  }
});

// ── POST /api/returns ─────────────────────────────────────────────────────────
app.post("/api/returns", submitLimiter, returnsUpload.array("attachments", 5), async (req, res) => {
  const metadata = JSON.stringify({
    ip: req.ip,
    user_agent: req.get("User-Agent"),
    referer: req.get("Referer") || "Direct",
  ...
  const { full_name, email_address, contact_number, order_number, items_to_return, reason } = req.body;
  const now = Date.now();
  const formLoadedAt = parseInt(req.body.form_loaded_at, 10);
  ...
  try {
    if (!TESTING_MODE) {
      const dup = await queryAsync(
        `SELECT id FROM return_requests WHERE email_address = ? AND order_number = ?
         AND created_at > DATE_SUB(NOW(), INTERVAL 2 MINUTE) LIMIT 1`,
        [email_address, order_number],
      );
      if (dup.length) {
        log("info", "return_duplicate_ignored", { email: email_address });
        return res.status(200).json({ success: true, message: "We already received this request. No need to submit again!" });
      }
    }
    ...
    const result = await queryAsync(
      `INSERT INTO return_requests
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [full_name, email_address, contact_number, order_number, items_to_return, reason, imagePaths, metadata],
    );

    log("info", "return_saved", { id: result.insertId, email: email_address, ip: req.ip, attachmentCount: req.files.length });

    res.json({ success: true, message: "Return request submitted! Please check your email for confirmation.", id: result.insertId });

    sendReturnEmails(
      { id: result.insertId, full_name, email_address, contact_number, order_number, items_to_return, reason },
      req.files,
      metadata,
    );
  } catch (err) {
    log("error", "return_insert_failed", { message: err.message, email: email_address });
...
app.get("/api/submissions", requireAdmin, async (req, res) => {
  ...
  if (search) {
    whereClauses.push("(full_name LIKE ? OR email_address LIKE ? OR id_number LIKE ?)");
    const like = `%${search}%`;
  ...
  try {
    const countRows = await queryAsync(`SELECT COUNT(*) AS total FROM applications${whereSql}`, whereParams);
    ...
    log("info", "submissions_listed", { page, limit, totalItems, search: search || null, date_from: dateFrom || null, date_to: dateTo || null, ip: req.ip });

    res.status(200).json({
      metadata: { total_items: totalItems, total_pages: Math.ceil(totalItems / limit) || 1, current_page: page },
      data: results,
    });
  } catch (err) {
...
app.patch("/api/submissions/:id/status", requireAdmin, async (req, res) => {
  ...
  try {
    const rows = await queryAsync("SELECT * FROM applications WHERE id = ?", [id]);
    ...
    if (status === "approved" && applicant.status !== "approved") {
      if (!shopifyConfigured()) {
        return res.status(503).json({ error: "Shopify is not configured. Set SHOPIFY_SHOP_DOMAIN and SHOPIFY_ADMIN_TOKEN." });
      }

      const { code, discountId, discountCodeId } = await createDiscountForApplicant(applicant);

      await queryAsync(
        `UPDATE applications
           SET status = 'approved', discount_code = ?, discount_id = ?, discount_code_id = ?, rejection_reason = NULL
           WHERE id = ?`,
        [code, discountId, discountCodeId, id],
      );

      log("info", "submission_approved", { id, code, discountId, ip: req.ip });
    ...
    // ── Revert approved (delete Shopify discount) ─────────────────────────────
    if (status !== "approved" && applicant.discount_id) {
      try {
        await deleteDiscount(applicant.discount_id);
      } catch (e) {
        log("warn", "shopify_delete_failed", { id, discountId: applicant.discount_id, message: e.message });
      }
      await queryAsync(
        `UPDATE applications
           SET status = ?, discount_code = NULL, discount_id = NULL, discount_code_id = NULL, rejection_reason = ?
           WHERE id = ?`,
    ...
    log("error", "status_update_failed", { id, status, message: err.message });
...
app.get("/api/submissions/:id/discount-usage", requireAdmin, async (req, res) => {
  ...
  try {
    const rows = await queryAsync(
      "SELECT discount_id, discount_code_id, discount_code FROM applications WHERE id = ?",
      [id],
    );
    if (!rows.length) return res.status(404).json({ error: "Not found" });

    const { discount_id, discount_code_id, discount_code } = rows[0];
    if (!discount_id || !discount_code_id) {
    ...
    const data = await shopifyGraphQL(query, { id: discount_id });
    const usageCount = data?.codeDiscountNode?.codeDiscount?.codes?.nodes?.[0]?.usageCount ?? 0;

    res.json({ code: discount_code, usage_count: usageCount, used: usageCount >= 1 });
  } catch (err) {
...
app.post("/api/auth/shopify", express.json(), async (req, res) => {
  try {
    ...
    const dashboardToken = jwt.sign(
      { shopOrigin, admin: true, exp: Math.floor(Date.now() / 1000) + 30 * 60 },
      secret,
    );
...
app.use((req, res, next) => {
  const origins = (process.env.ALLOWED_ORIGINS || "https://admin.shopify.com https://*.myshopify.com")
    .split(",")
    .map((o) => o.trim())
    .join(" ");
  res.setHeader("Content-Security-Policy", `frame-ancestors 'self' ${origins};`);
...
app.get("/api/contact-submissions", requireAdmin, async (req, res) => {
  ...
  if (/^\d{4}-\d{2}-\d{2}$/.test(dateFrom)) { whereClauses.push("created_at >= ?"); whereParams.push(`${dateFrom} 00:00:00`); }
  if (/^\d{4}-\d{2}-\d{2}$/.test(dateTo)) { whereClauses.push("created_at <= ?"); whereParams.push(`${dateTo} 23:59:59`); }
  const whereSql = whereClauses.length ? ` WHERE ${whereClauses.join(" AND ")}` : "";

  try {
    const countRows = await queryAsync(`SELECT COUNT(*) AS total FROM contact_submissions${whereSql}`, whereParams);
    const totalItems = countRows[0].total;
    const data = await queryAsync(
      `SELECT * FROM contact_submissions${whereSql} ORDER BY created_at DESC LIMIT ? OFFSET ?`,
      [...whereParams, limit, offset],
    );
    res.json({ metadata: { total_items: totalItems, total_pages: Math.ceil(totalItems / limit) || 1, current_page: page }, data });
  } catch (err) {
...
app.patch("/api/contact-submissions/:id/status", requireAdmin, async (req, res) => {
  ...
  try {
    const result = await queryAsync("UPDATE contact_submissions SET status = ? WHERE id = ?", [status, id]);
...
app.get("/api/return-requests", requireAdmin, async (req, res) => {
  ...
  const whereParams = [];
  if (search) {
    whereClauses.push("(full_name LIKE ? OR email_address LIKE ? OR order_number LIKE ? OR items_to_return LIKE ?)");
    const like = `%${search}%`;
  ...
  try {
    const countRows = await queryAsync(`SELECT COUNT(*) AS total FROM return_requests${whereSql}`, whereParams);
    const totalItems = countRows[0].total;
    const data = await queryAsync(
...
app.patch("/api/return-requests/:id/status", requireAdmin, async (req, res) => {
  ...
  try {
    ...
    if (!rows.length) return res.status(404).json({ error: "Return request not found" });
    ...
    if (status === "approved" && returnReq.status !== "approved") sendReturnApprovalEmail(returnReq);
    if (status === "rejected" && !wasRejected) sendReturnRejectionEmail(returnReq, trimmedReason);
